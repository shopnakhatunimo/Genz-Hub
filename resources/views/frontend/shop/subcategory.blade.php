...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6 text-sm text-gray-500">
        ...
        <a href="{{ route('category', $subcategory->category->slug) }}" class="hover:text-primary-600">{{ $subcategory->category->name ?? '' }}</a>
        <span class="mx-2">/</span>
        <span class="text-gray-800">{{ $subcategory->name }}</span>
    </div>
    <h1 class="text-2xl font-bold mb-6">{{ $subcategory->name }}</h1>
    @if($products->count() > 0)
    ...
    <div class="text-center py-16">
        <p class="text-gray-500 text-lg">এই সাব-ক্যাটাগরিতে কোনো পণ্য নেই।</p>
